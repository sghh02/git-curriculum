...
目印（スクショ不要）：`File Status` 画面の中央に `Unstaged files / Staged files` が表示され、ファイルをクリックすると差分（diff）が見られます。

もし `Unstaged files` に何も出ないとき：
- VS Code で保存できているか確認（タブの ● が消えているか）
...
目印（スクショ不要）：`History` ではコミット一覧（メッセージ/日時）が並び、選ぶと変更内容（diff）が表示されます。
...
目印（スクショ不要）：`Push` ダイアログで “チェックが付いているブランチ” が送られます。`feature/...` にチェックがあるか必ず確認します。
...
作り方（スクショ不要）：
- VS Code でリポジトリ直下に `.gitignore` という名前のファイルを作る（拡張子なし）
...
目印（スクショ不要）：`History` のコミット行を右クリックするとメニューが出ます。`Reverse commit` / `Revert` は「取り消し用の新しいコミットを作る」安全な方法です。

## 5. AIに聞いてみよう（質問例）
このページを開いたまま、下のテンプレをコピペして質問してみてください（文脈を AI が見たうえで答えられます）。

**コピペ用テンプレ（コミット/プッシュ）**
```md
初心者です。いま「03 add/commit/pushとログの読み方」を開いています。

今いるブランチ：（Sourcetreeで太字/チェックのブランチ名）
File Statusの状態：
- Unstaged files：（ある/ない）
- Staged files：（ある/ない）
やりたいこと：（例）コミットしたい / プッシュしたい / 直前のコミットを取り消したい
起きていること：（例）Commitが押せない / Pushで失敗する / 余計なファイルが混ざる
エラー全文：（あればそのまま）
...
- 「`3-6 プッシュ` で、Push画面のどこにチェックを入れる？“送るブランチ” を間違えない確認ポイントを3つだけ教えて（スクショは任意）」
