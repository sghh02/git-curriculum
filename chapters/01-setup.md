...
- 前のAIプログラムの最終成果物を、後のPR章で `engineer-design.md` として使える状態にする
- 次章のために `develop` を作成して Push できる
- 次章で最初のコミットを始められる状態まで土台を整えられる
...
4. `engineer-design.md` の元になる内容を準備する
5. `develop` を作る

この5つが終われば、次章に進めます。  
`main` の保護や Diff/Merge の設定は、**できれば良いおすすめ設定** です。先に進めることを優先してください。

ここではまだ「提出用の最初のコミット」は行いません。  
先に GitHub / Sourcetree / フォルダの土台をそろえて、次章で落ち着いてコミットします。
...
このプログラムでは、Sourcetree の画面ラベルを **英語表記ベース** で書いています。
日本語表示のままでも進められますが、迷う場合は英語表示にそろえるか、第8章の「表示の読み替え（英語 / 日本語）」を参照してください。

Sourcetree に表示言語の切替項目がある場合は、`Preferences` / `Options` 付近の `Language` から日本語に変更して再起動します。  
ただし、OS やバージョンで見え方が違うことがあるので、見つからないときは公式情報とAIを使って「自分の画面ではどの項目に当たるか」を確認しながら進めてください。
...
### 5-5. `engineer-design.md` の元になる内容を準備する

このGitプログラムでは、前のAIプログラムで作成した **自分のエンジニア設計書** を PR 練習の題材として使います。

1. クローンしたフォルダをお使いのエディタで開く
2. 前のAIプログラムで作った最終成果物を開く
3. PR章ですぐ貼り付けられるように、内容を見返せる状態にしておく

まだ完成度が低くても問題ありません。  
次章以降で、AIに相談しながら少しずつ改善していきます。

ここで大事なのは、「完成版を置く」ことではなく、**これから Git で育てていく元の文章を手元に準備しておく**ことです。

この章では、まだリポジトリ内に `engineer-design.md` を作らなくてOKです。
次章と次々章では、まず練習用ファイルでコミット・ブランチ操作に慣れます。
`engineer-design.md` を追加するのは、PR章で `feature/04-pull-request` を作ってからにします。

**できた状態**：前プログラムの最終成果物を見返せる状態で、PR章ですぐ使える。

### 5-6. 次章の準備として `develop` を作る

このプログラムでは、`main` とは別に `develop` を使って進めます。最初に1回だけ作成しておきましょう。
...
- [ ] 前AIプログラムの最終成果物を、PR章ですぐ使える状態にできた
