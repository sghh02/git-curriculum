...
目印（スクショ不要）：`base` と `compare` は PR 作成画面の上部にあります（左が base、右が compare）。間違えてもドロップダウンを開いて選び直せます。
...
- 次の観点をチェックする
  ...
  - 誤字脱字、見出し、読みやすさはOK？
  - 「課題の要件」を満たしている？
目印（スクショ不要）：PR 画面上部のタブ列に `Conversation / Commits / Files changed` が並びます。`Files changed` で差分（diff）が読めます。
