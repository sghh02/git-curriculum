...
- `main` を元に戻す（Reset など）は強い操作なので、授業ルールに合わせてメンターに確認するのがおすすめです
...
### 5-1. まず確認（ここが9割）
- Sourcetree で GitHub にログインしている？（`Preferences` → `Accounts` を確認）
- GitHub の `Code` → `HTTPS` の URL（`https://github.com/...`）を使っている？
- リポジトリの URL が正しい？（自分の課題リポジトリか確認）

### 5-2. それでもダメなとき
- Sourcetree を再起動して、もう一度ログインを試す
- ブラウザで GitHub にログインできるか確認
- 会社/学校ネットワークで制限されている可能性（その場合はメンターに相談）
...
1. **今プッシュしたいブランチ**（例：`feature/課題名`）をチェックアウト
...
- `feature/課題名` を **プッシュしていない** 可能性が高いです
  - Sourcetree で `feature/課題名` をチェックアウト → `Push`
...
2. お使いのエディタ（例：VS Code）でファイルを整える
...
- 「コンフリクトが出た。表示されている内容（エラー文や衝突部分）を書くので、"どっちを残すか" の判断の考え方と、Sourcetree→エディタ→Sourcetree の手順を確認して」
