...
今回の題材で言うと、`engineer-design.md` を直しているつもりなのに、別のファイルまで変更に入っていたら一度止まって確認します。

そのうえで、次の順番で切り分けると迷いにくいです。

1. **ローカルの問題か**：ブランチ / 未コミット変更 / コミットの有無
2. **GitHub との通信か**：Push / Pull / 認証
3. **提出の問題か**：PR の base / compare / Conflicts
...
たとえば `engineer-design.md` を直してコミットしたのに GitHub にブランチが見えない場合は、まず「いま本当に `feature/...` にいるか」と「Push できているか」を確認します。
...
PR が作れないときは、「Push まで終わっているか」を先に見ます。
