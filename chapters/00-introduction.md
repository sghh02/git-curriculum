...
あなたのPC（ローカル）
  ├ VS Code：編集する
  └ Sourcetree：commit / branch / push / pull をクリックで操作する（リモコン）
...
2. スクショは任意です（面倒なら、画面の文字をそのまま書けばOK）:
   - Mac：`Shift + Command + 4`
...
このテンプレは、メモアプリにそのままコピペして保存しておくのがおすすめです（スクショ不要）。
